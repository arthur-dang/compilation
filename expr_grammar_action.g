...
non-terminals<struct ast_node*> S ARG STATEMENT AFFECTATION RETURN PRINT IFELSE ELSE WHILE EXPR CMP TERM FACTOR FACTIR
non-terminals<struct list*> FUNCTION ARGS ARGP IBLOCK CMPP TERMP FACTORP
rules
...
WHILE -> SYM_WHILE SYM_LPARENTHESIS EXPR SYM_RPARENTHESIS SYM_LBRACE IBLOCK SYM_RBRACE { return make_node(AST_IWHILE, cons($3, cons(make_node(AST_IBLOCK, $6), NULL))); }
